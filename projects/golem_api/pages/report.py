...
EXECUTION_LAST_EXECUTIONS_ENDPOINT = '/report/execution/last-executions'
TEST_STATUS_ENDPOINT = '/report/test/status'
EXECUTION_ENDPOINT = '/report/execution'
DELETE_EXECUTION_ENDPOINT = '/report/execution'
DELETE_EXECUTION_TIMESTAMP_ENDPOINT = '/report/execution/timestamp'
...
def get_execution_last_executions(project, execution, user=None):
    params = {'project': project, 'execution': execution}
    return requests.get(url(EXECUTION_LAST_EXECUTIONS_ENDPOINT),
                        headers=headers(user), params=params)
...
def delete_execution(project, execution, user=None):
    json_ = {
        'project': project,
        'execution': execution
    }
    return requests.delete(url(DELETE_EXECUTION_ENDPOINT), headers=headers(user), json=json_)
...
def delete_execution_timestamp(project, execution, timestamp, user=None):
    ...
    return requests.delete(url(DELETE_EXECUTION_TIMESTAMP_ENDPOINT),
                           headers=headers(user), json=json_)
